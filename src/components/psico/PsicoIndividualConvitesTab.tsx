...
export default function PsicoIndividualConvitesTab({ avaliacaoId }: { avaliacaoId: string }) {
  ...
  async function carregar(somenteConsulta: boolean) {
    setLoading(true);
    try {
      const { data, error } = await supabase.functions.invoke("psico-individual-invite-token", {
        body: { avaliacao_id: avaliacaoId, somente_consulta: somenteConsulta },
      });
      if (error) throw error;
      setConvites(((data as any)?.convites || []) as ConviteResp[]);
    } catch (e: any) {
      toast({
        title: somenteConsulta ? "Falha ao consultar convites" : "Falha ao gerar convites",
  ...
  useEffect(() => {
    setConvites(null);
    void carregar(true);
    // A consulta precisa ser refeita ao trocar de avaliação.
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <Button size="sm" onClick={() => carregar(false)} disabled={loading}>
          <RefreshCw className={`h-4 w-4 mr-1 ${loading ? "animate-spin" : ""}`} />
          {convites?.length ? "Atualizar" : "Gerar links"}
        </Button>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        {convites === null ? (
          <p className="text-sm text-muted-foreground">Consultando convites existentes…</p>
        ) : convites.length === 0 ? (
