// Painel de precificação exibido inline dentro do Sheet do ProposalEditor.
// (Distinto de `PricingPanel.tsx`, que é uma variante independente.)
...
import {
  computePricing, statusMargemMeta,
  type PricingInput, type CustoDiretoRow, type HoraTecnicaRow,
  CUSTO_CATEGORIAS, ATIVIDADE_CATEGORIAS,
  normalizarCustosDiretos, normalizarHorasTecnicas,
} from "@/lib/pricing";
import { Row } from "./ProposalSummary";

const newId = () => Math.random().toString(36).slice(2, 10);

export default function InlinePricingPanel({ item, existing, params, clientFuncionarios, onSave }: any) {
      ...
      <section className="grid md:grid-cols-2 gap-4">
        <div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <MiniPct label="Imposto (%)" v={draft.aliquota_imposto} onChange={(v: any) => setDraft({ ...draft, aliquota_imposto: Number(v) || 0 })} />
            <MiniPct label="Margem desejada (%)" v={draft.margem_desejada} onChange={(v: any) => setDraft({ ...draft, margem_desejada: Number(v) || 0 })} />
            <Mini label="Lucro adicional (R$)" v={draft.lucro_desejado} onChange={(v: any) => setDraft({ ...draft, lucro_desejado: Number(v) || 0 })} />
            <Mini label="Desconto (R$)" v={draft.desconto_comercial} onChange={(v: any) => setDraft({ ...draft, desconto_comercial: Number(v) || 0 })} />
          </div>
        </div>
