...
export default function Contratos() {
  ...
  const load = async () => {
    const [c, cl] = await Promise.all([
      supabase.from("financeiro_contratos").select("*, proposals(numero, titulo)").order("data_aprovacao", { ascending: false }),
      supabase.from("clients").select("id, razao_social, nome_fantasia"),
    ]);
    setRows(c.data||[]); setClients(cl.data||[]);
  };
  ...
  return (
    <div>
      ...
      <div className="p-6 space-y-4">
        <div className="flex gap-2"><div className="relative flex-1 max-w-md"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground"/>
          <Input className="pl-9" placeholder="Buscar por cliente, número ou título…" value={q} onChange={e=>setQ(e.target.value)} /></div></div>
        <Card className="overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {filtered.length === 0 && <tr><td colSpan={7} className="text-center py-10 text-muted-foreground">Nenhum contrato encontrado. Aprove uma proposta para gerar.</td></tr>}
            </tbody>
          </table>
        </Card>
      </div>
