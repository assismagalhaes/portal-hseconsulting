...
export const REPORT_MODEL_VERSION = "1.5.1";
...
export function PsychosocialReportDocument({ snapshot, codigoRafp, codigoRev, codigoValidacao, cliente, empresa, dataEmissao, preview = false, qrDataUrl, assinaturaDataUrl }: any) {
  ...
  return <Document title={`Relatório ${codigoRafp} ${codigoRev}`} author="HSE Consulting" subject="Avaliação de Fatores Psicossociais" creator="Portal HSE" producer="Portal HSE">
    ...
    <Page size="A4" style={styles.page}>{headerFooter}<Text style={styles.kicker}>PARA A DIREÇÃO E LIDERANÇAS</Text><Text style={styles.h1}>Resumo executivo</Text><Text style={styles.intro}>O que a avaliação mostrou sobre o ambiente de trabalho, onde concentrar a atenção primeiro e como organizar a resposta da empresa nos próximos 12 meses.</Text>
      ...
      <View style={styles.row}><Kpi value={responseCount} label="Pessoas que responderam" /><Kpi value={significant.length} label="Fatores que exigem ação" /><Kpi value={highest ? riskLabel(highest.prioridade) : "Monitoramento"} label="Prioridade mais alta" hint={`Começar a agir em até ${priorityDeadline} dias`} last color={highest ? riskColor(highest.prioridade) : REPORT_COLORS.green} /></View>
      ...
      <View style={styles.infoPanel}><Text style={styles.h3}>Como ler este relatório</Text><Text style={styles.note}>Os resultados mostram a percepção coletiva das equipes sobre como o trabalho está organizado — não são diagnóstico psicológico de pessoas nem apontam indivíduos. Para transformar os números em melhorias, a empresa precisa comparar cada achado com o trabalho real por meio de escuta das equipes, observação e análise técnica.</Text><Text style={[styles.note, { marginTop: 6 }]}><Text style={{ fontFamily: "Helvetica-Bold" }}>Glossário rápido: </Text>Índice: média das respostas de 0 (favorável) a 4 (muito desfavorável). Atenção geral: quantas pessoas responderam em nível médio, alto ou crítico. Atenção intensa: respostas em nível alto ou crítico. Situação crítica: respostas na faixa mais grave. Fator que exige ação: aquele que ultrapassou os limites definidos na metodologia.</Text></View>
    </Page>

    <Page size="A4" style={styles.page}>{headerFooter}<Text style={styles.kicker}>PANORAMA GERAL</Text><Text style={styles.h1}>Como cada fator foi avaliado</Text><Text style={styles.intro}>O gráfico mostra o índice médio de cada fator numa escala de 0 (favorável) a 4 (muito desfavorável). Quanto mais à direita, mais atenção o fator exige. A decisão de agir considera também quantas pessoas responderam nas faixas mais críticas — por isso a tabela abaixo é a referência principal.</Text><ScoreChart factors={factors} /><Text style={styles.h2}>Quadro de referência para a decisão</Text><SignificanceTable factors={factors} /><Text style={[styles.note, { marginTop: 7 }]}>Atenção geral: pessoas nas faixas média, alta ou crítica. Atenção intensa: pessoas nas faixas alta ou crítica. Situação crítica: pessoas na faixa mais grave. Um fator exige ação quando cruza pelo menos um dos limites definidos na metodologia.</Text></Page>

    {factorPages.map((pageFactors, pageIndex) => <Page key={`factor-analysis-${pageIndex}`} size="A4" style={styles.page}>{headerFooter}<Text style={styles.kicker}>ANÁLISE POR FATOR</Text><Text style={styles.h1}>{significant.length ? "Fatores que exigem atenção" : "Monitoramento preventivo"}{pageIndex ? " · continuação" : ""}</Text><Text style={styles.intro}>{pageIndex ? "Continuação da análise, mantendo indicadores, interpretação e caminhos de ação no mesmo bloco." : significant.length ? "Para cada fator, apresentamos o resultado, o que ele indica, as perguntas que mais pesaram e o caminho recomendado de ação." : "Como nenhum fator ultrapassou os limites, o foco é preservar as condições atuais e acompanhar mudanças no trabalho."}</Text>
      {pageFactors.map((factor) => { const qs = (questionsByFactor.get(factor?.fator_codigo) || []).filter(questionHasAttention).sort((a, b) => Number(b?.percentual_critico || 0) - Number(a?.percentual_critico || 0) || Number(b?.percentual_alto_critico || 0) - Number(a?.percentual_alto_critico || 0) || Number(b?.percentual_desfavoravel || 0) - Number(a?.percentual_desfavoravel || 0)).slice(0, 3); const relatedActions = actions.filter((a) => list(a?.fatores).includes(factor?.fator_codigo)); return <View key={factor?.fator_codigo} style={[styles.factorCard, { borderTopColor: riskColor(factor?.prioridade || factor?.classificacao) }]} wrap={false}><View style={styles.factorHeader}><Text style={styles.factorName}>{factorName(factor)}</Text><Text style={[styles.pill, { backgroundColor: riskColor(factor?.prioridade) }]}>Prioridade {riskLabel(factor?.prioridade)}</Text></View><View style={styles.metricRow}><Text style={styles.metric}>Índice <Text style={styles.metricValue}>{score(factor?.score_medio)}</Text></Text><Text style={styles.metric}>Atenção geral <Text style={styles.metricValue}>{pct(factor?.percentual_medio_alto_critico)}</Text></Text><Text style={styles.metric}>Atenção intensa <Text style={styles.metricValue}>{pct(factor?.percentual_alto_critico)}</Text></Text><Text style={styles.metric}>Situação crítica <Text style={styles.metricValue}>{pct(factor?.percentual_critico)}</Text></Text></View><Text><Text style={{ fontFamily: "Helvetica-Bold" }}>O que isso significa: </Text>{clean(factor?.observacao || factor?.justificativa, `${factorName(factor)} ${factor?.significativo ? "atingiu os limites de atenção e precisa ser analisado à luz do trabalho real" : "permaneceu dentro dos limites aceitáveis no período"}.`)}</Text><Text style={{ marginTop: 4 }}><Text style={{ fontFamily: "Helvetica-Bold" }}>Caminho recomendado: </Text>{FACTOR_DIRECTION[factor?.fator_codigo] || "Comparar os indicadores com o trabalho real e acompanhar se as medidas adotadas produzem efeito."}</Text>{qs.length > 0 && <View style={styles.questionAttention}><Text style={styles.h3}>Perguntas que mais pesaram neste fator</Text>{qs.map((q) => <Text key={q.numero} style={styles.note}>Pergunta {q.numero} · {clean(q.texto)} — {pct(q.percentual_critico)} em situação crítica e {pct(q.percentual_alto_critico)} em atenção intensa</Text>)}</View>}{relatedActions.length > 0 && <Text style={[styles.note, { marginTop: 4 }]}>Ações previstas no plano: {relatedActions.map((a) => fixTypos(a.titulo)).join("; ")}.</Text>}</View>; })}
    </Page>)}

    {priorityQuestionPages.map((groups, pageIndex) => <Page key={`priority-questions-${pageIndex}`} size="A4" style={styles.page}>{headerFooter}<Text style={styles.kicker}>ONDE CONCENTRAR A ATENÇÃO</Text><Text style={styles.h1}>Perguntas que mais pesaram{pageIndex ? " · continuação" : ""}</Text>{pageIndex === 0 && <Text style={styles.intro}>Selecionamos até cinco perguntas por fator com maior peso nas respostas críticas e de atenção intensa. Todos os dados são coletivos; nenhuma resposta individual é apresentada e ninguém é identificado.</Text>}{groups.map(({ factor, questions: groupQuestions }) => <View key={factor?.fator_codigo} style={styles.questionGroup} wrap={false}><View style={styles.questionHeader}><Text style={styles.questionTitle}>{factorName(factor)}</Text></View><QuestionTable questions={groupQuestions} /></View>)}</Page>)}
