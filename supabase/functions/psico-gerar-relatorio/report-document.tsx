...
function BulletList({ items }: { items: string[] }) { return <>{items.map((item, index) => <View style={styles.bullet} key={index}><Text style={styles.bulletMark}>•</Text><Text style={styles.bulletText}>{item}</Text></View>)}</>; }
...
export function PsychosocialReportDocument({ snapshot, codigoRafp, codigoRev, codigoValidacao, cliente, empresa, dataEmissao, preview = false, qrDataUrl, assinaturaDataUrl }: any) {
  ...
  const questionGroups = factors.map((factor) => ({ factor, questions: questions.filter((q) => q?.fator_codigo === factor?.fator_codigo) })).filter((group) => group.questions.length);
