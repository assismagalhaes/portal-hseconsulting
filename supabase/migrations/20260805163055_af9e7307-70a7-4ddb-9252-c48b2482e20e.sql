-- Responsaveis tecnicos possuem acesso integral ao modulo psicossocial por
-- can_see_psico(). Esta RPC ainda usava o gate legado can_see_internal().
...
DECLARE
  ...
  v_exists boolean;
BEGIN
  IF auth.uid() IS NULL OR NOT public.can_see_psico(auth.uid()) THEN
  ...
  LOOP
    ...
    SELECT EXISTS (
      SELECT 1
        FROM public.psico_avaliacoes
       WHERE link_publico_token = v_token
    ) INTO v_exists;
