-- HSE-PSICO-REL-1.4.0
-- Parecer conclusivo estruturado, assinatura gráfica privada e snapshot
-- agregado completo (sem respostas individuais ou identificação de participantes).
...
VALUES ('psico-assinaturas', 'psico-assinaturas', false, 2097152, ARRAY['image/png', 'image/jpeg'])
ON CONFLICT (id) DO UPDATE SET
  public = false,
  file_size_limit = EXCLUDED.file_size_limit,
  allowed_mime_types = EXCLUDED.allowed_mime_types;

-- Migração compatível: pareceres já aprovados são estruturados a partir do texto
-- humano existente, sem reabrir nem alterar a conclusão histórica.
...
-- O responsável e a configuração de assinatura são congelados na aprovação.
...
-- O sanitizador preserva somente a identificação profissional e a referência
-- segura ao asset privado congelado; contatos e dados de respondentes continuam proibidos.
...
-- Configuração institucional centralizada usada pelo renderer.
