...
BEGIN
  ...
  LOOP
    -- Malformacao estrutural segue para a autoridade estrita.
    ...
    -- Catalogo invalido segue intacto para falhar fechado.
    IF NOT FOUND THEN
      _selecoes_sanitizadas := _selecoes_sanitizadas || jsonb_build_array(_sel);
      CONTINUE;
    END IF;
    ...
    LOOP
      ...
      -- Fator inexistente ou tratamento invalido segue para falhar fechado.
      ...
      -- Vínculo gerado pela IA para fator sem ação é descartado. A RPC estrita
      -- permanece intacta e ainda rejeita esse vínculo quando chamada direto.
      ...
      -- Monitoramento e opcional: aceita somente uma medida essencial por fator.
