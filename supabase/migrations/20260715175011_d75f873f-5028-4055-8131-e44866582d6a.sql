...
BEGIN
  ...
  IF v_av.questionario_versao_id IS NOT NULL THEN
    ...
    -- valores corretos do enum psico_questionario_status: em_configuracao | publicada | arquivada
    IF NOT FOUND OR v_qv.status::text NOT IN ('publicada','arquivada') THEN
  ...
  RETURN jsonb_build_object(
    ...
    'integridade_ok', v_itens_por_resposta_ok AND (v_qtd_convites_resp = v_qtd_resp) AND (v_qtd_itens = v_itens_esperados),
...
-- Reforça grants e revoga acesso público/anônimo
REVOKE ALL ON FUNCTION public.psico_validar_processamento_resultados(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.psico_validar_processamento_resultados(uuid) FROM anon;
...
REVOKE ALL ON FUNCTION public.psico_resumo_coleta(uuid) FROM anon;
