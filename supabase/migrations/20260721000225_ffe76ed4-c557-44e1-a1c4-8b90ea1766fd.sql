...
-- ============================================================
-- 1) Materializador de respostas do modo público anônimo
-- ============================================================
CREATE OR REPLACE FUNCTION public.psico_materializar_respostas_publicas(p_avaliacao_id uuid)
RETURNS int
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_pergunta_id uuid; v_opcao_id uuid;
  v_data date;
  v_num int;
BEGIN
  ...
  IF v_av.modo_coleta IS DISTINCT FROM 'publico_anonimo' THEN
    RETURN 0;
  END IF;

  -- Idempotência: se já existir alguma resposta materializada, não repete.
  IF EXISTS (SELECT 1 FROM public.psico_respostas WHERE avaliacao_id = p_avaliacao_id) THEN
    RETURN 0;
  END IF;

  FOR v_row IN
    SELECT * FROM public.psico_respostas_publicas WHERE avaliacao_id = p_avaliacao_id
  LOOP
    -- Ignora envios com número de itens diferente de 35 (não devem ocorrer, mas protege o pipeline).
    IF (SELECT count(*) FROM jsonb_each_text(v_row.respostas)) <> 35 THEN
      CONTINUE;
    END IF;

    v_data := COALESCE(v_row.created_at::date, now()::date);
    ...
    INSERT INTO public.psico_respostas (
      ...
      data_resposta, quantidade_itens, origem_registro
    ) VALUES (
      v_av.id, v_av.questionario_versao_id, v_av.metodologia_versao_id,
      v_row.funcao, v_row.setor, v_row.unidade,
      v_row.funcao_normalizada, v_row.setor_normalizada, v_row.unidade_normalizada,
      v_data, 35, 'portal'
    ) RETURNING id INTO v_resp_id;

    FOR v_key, v_val IN SELECT key, value FROM jsonb_each_text(v_row.respostas) LOOP
      v_num := NULLIF(v_key, '')::int;

      SELECT id INTO v_pergunta_id
        FROM public.psico_perguntas
       WHERE questionario_versao_id = v_av.questionario_versao_id
         AND numero = v_num AND ativa = true;
      IF v_pergunta_id IS NULL THEN
        RAISE EXCEPTION 'Pergunta % não encontrada no questionário vinculado.', v_num
          USING ERRCODE = 'foreign_key_violation';
      END IF;

      SELECT id INTO v_opcao_id
        FROM public.psico_opcoes_resposta
       WHERE metodologia_versao_id = v_av.metodologia_versao_id
         AND codigo = v_val AND ativo = true;
      IF v_opcao_id IS NULL THEN
        RAISE EXCEPTION 'Opção "%": não encontrada na metodologia vinculada.', v_val
          USING ERRCODE = 'foreign_key_violation';
      END IF;

      INSERT INTO public.psico_respostas_itens (resposta_id, pergunta_id, opcao_resposta_id)
      VALUES (v_resp_id, v_pergunta_id, v_opcao_id);
    END LOOP;
...
REVOKE ALL ON FUNCTION public.psico_materializar_respostas_publicas(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.psico_materializar_respostas_publicas(uuid) TO service_role;

-- ============================================================
-- 2) Encerrar coleta: materializa automaticamente no modo público
-- ============================================================
CREATE OR REPLACE FUNCTION public.psico_encerrar_coleta(p_avaliacao_id uuid, p_confirmacao text, p_motivo text DEFAULT NULL::text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  IF v_av.modo_coleta = 'publico_anonimo' THEN
    v_materializadas := public.psico_materializar_respostas_publicas(p_avaliacao_id);
  END IF;
  ...
  VALUES ('avaliacao', p_avaliacao_id, 'coleta_encerrada',
    jsonb_build_object(
      ...
      'materializadas_publicas', v_materializadas,
      'modo_coleta', v_av.modo_coleta,
      'motivo_informado', p_motivo IS NOT NULL
    ));

  RETURN jsonb_build_object(
    'status','coleta_encerrada',
    'respondidos', v_resp,
    'expirados', v_expirados,
    'materializadas_publicas', v_materializadas
  );
...
-- ============================================================
-- 3) Validação de processamento: dispensa convites no modo público
-- ============================================================
...
BEGIN
  ...
  v_publico_anonimo  := v_av.modo_coleta   = 'publico_anonimo';
  ...
  IF v_publico_anonimo THEN
    IF v_qtd_convites_resp <> 0 THEN
      v_avisos := v_avisos || jsonb_build_array(
        format('Modo público anônimo possui %s convite(s) legados, ignorados na validação.',
               v_qtd_convites_resp));
    END IF;
  ELSIF NOT v_importacao_bruta AND v_qtd_convites_resp <> v_qtd_resp THEN
  ...
  RETURN jsonb_build_object(
    ...
    'integridade_ok', v_itens_por_resposta_ok
      AND (v_publico_anonimo OR v_importacao_bruta OR v_qtd_convites_resp = v_qtd_resp)
