...
BEGIN
  ...
  IF _av.origem_coleta = 'importacao_agregada' THEN
    SELECT jsonb_agg(jsonb_build_object(
      'pergunta_id', da.pergunta_id, 'numero', da.numero,
  ...
  SELECT jsonb_build_object(
    ...
    'plano', (
      SELECT jsonb_build_object(
        'status', p.status, 'quantidade', p.quantidade_itens,
        'itens', (
          SELECT jsonb_agg(jsonb_build_object(
            'id', i.id, 'titulo', i.titulo,
            ...
            'fatores', (SELECT array_agg(fator_codigo) FROM public.psico_plano_item_fatores WHERE plano_item_id = i.id)
          ) ORDER BY i.ordem)
          FROM public.psico_plano_acao_itens i
          WHERE i.plano_id = p.id AND i.selecionado = true
        )
      )
