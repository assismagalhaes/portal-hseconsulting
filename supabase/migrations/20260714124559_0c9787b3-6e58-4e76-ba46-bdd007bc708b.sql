-- 1) Default de 7 dias para novos aceites
ALTER TABLE public.proposal_aceites
  ALTER COLUMN expires_at SET DEFAULT (now() + interval '7 days');

-- 2) Backfill: links pendentes sem data de expiração ganham 7 dias a partir do created_at
UPDATE public.proposal_aceites
   SET expires_at = created_at + interval '7 days'
 WHERE expires_at IS NULL
   AND status = 'pendente';

-- 3) RPC: auto-expira ao carregar o link
...
BEGIN
  ...
  -- Auto-expira se estiver pendente e passou do prazo
