...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'plano', (
      SELECT jsonb_build_object(
        'status', pa.status,
        'quantidade', pa.quantidade_itens,
        'titulo', pa.titulo,
        ...
        'itens', (
          SELECT jsonb_agg(jsonb_build_object(
            'id', pai.id,
            'titulo', pai.titulo,
            'acao', pai.acao_recomendada,
            'nivel', pai.nivel_recomendacao,
            'grupo', pai.grupo_transversal,
            'prioridade', pai.prioridade,
            'prazo_dias', pai.prazo_sugerido_dias,
            'responsavel', COALESCE(pai.responsavel_definido, array_to_string(pai.responsaveis_sugeridos, ', ')),
            'evidencias', pai.evidencias_recomendadas,
            'abrangencia', pai.abrangencia_rotulo,
            'fatores', (
              SELECT array_agg(pif.fator_codigo ORDER BY pif.fator_codigo)
              FROM public.psico_plano_item_fatores pif
              WHERE pif.plano_item_id = pai.id
            )
          ) ORDER BY pai.ordem, pai.criado_em)
          FROM public.psico_plano_acao_itens pai
          WHERE pai.plano_id = pa.id
            AND pai.selecionado = true
        )
      )
      FROM public.psico_planos_acao pa
      WHERE pa.revisao_id = _rev.id
      ORDER BY pa.aprovado_em DESC NULLS LAST, pa.criado_em DESC
      LIMIT 1
    ),
    'agregado', _agregado
  ) INTO _resultado;
