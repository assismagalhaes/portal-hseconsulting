...
const output = Deno.args[0] || "output/pdf/v1.6.1";
await Deno.mkdir(output, { recursive: true });
for (const [index, scenario] of scenarios.entries()) {
  const fs = factors(scenario.significant);
  const snapshot = {
    modelo: { codigo: "HSE-PSICO-REL-1.0", versao: "1.6.1" },
  ...
  const buffer = await renderToBuffer(<PsychosocialReportDocument snapshot={snapshot} codigoRafp={`RAFP-QA-${String(index + 1).padStart(4, "0")}`} codigoRev={scenario.revision || "R00"} codigoValidacao="QA-V1-6-1" cliente={{ nome: clientName, razao_social: clientName, cnpj_cpf: "12345678000190", endereco: "Avenida da Homologação", numero: "100", bairro: "Centro", cidade: "Fortaleza", uf: "CE", cep: "60000-000" }} empresa={{ telefone: "(85) 9.9142-6534", email: "contato@hseconsulting.com.br", site: "hseconsulting.com.br" }} dataEmissao="2026-07-20T16:00:00Z" assinaturaDataUrl={scenario.signature ? HSE_LOGO_GREEN_DATA_URL : undefined} clientLogoDataUrl={scenario.name === "01-sem-fator-significativo" ? HSE_LOGO_GREEN_DATA_URL : undefined} />);
